...
// ---------- OpenAI 客户端（兼容 MiniMax M3 走 shuyanai.com 平台） ----------
function makeClient() {
  const apiKey = process.env.MINIMAX_API_KEY || process.env.OPENAI_API_KEY;
  const baseURL = process.env.MINIMAX_BASE_URL || process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1';
  if (!apiKey) {
    throw new Error('未配置 API Key。请在 .env 中设置 MINIMAX_API_KEY（推荐用 MiniMax M3 走 shuyanai.com 平台）');
  }
...
async function fetchSite(url, onLog) {
  ...
  try {
    const r = await fetch(url, {
      signal: ctrl.signal,
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) KiwiiBrandLab/1.1',
...
const SYSTEM_PROMPT = `你是 Kiwii Brand Lab 的资深品牌视觉策略师，专长是消费品的视觉解构与对标分析。
...
async function analyzeWithLLM({ name, url, text, meta, onLog }) {
  ...
  // 解析 JSON（防御性：剥 thinking block + markdown 围栏 + 平衡括号提取）
  const cleaned = cleanLLMOutput(raw);
  let data = null;
  try {
    data = JSON.parse(cleaned);
  } catch {
    onLog?.({ level: 'warn', msg: '⚠ LLM 返回非 JSON，已尝试修复…' });
    data = extractFirstJSON(cleaned);
  }
  if (!data) {
    throw new Error('LLM 返回无法解析为 JSON (前 200 字符): ' + cleaned.slice(0, 200));
  }
...
// ---------- 主入口 ----------
export async function runResearch({ raw, onLog }) {
  ...
  const data = await analyzeWithLLM({ name, url, text, meta, onLog });
